...
\section{Conception de la pince}
    ...
    \subsection{Considérations mathématiques}
        ...
        \subsubsection{Discussion sur le cahier des charges à respecter}
            ...
            Pour satisfaire aux exigences du cahier des charges ci-dessus, on cherche les valeurs des paramètres $b,\ a,\ \theta_0$ : 
        ...
        Il nous reste maintenant à augmenter d'un rapport $\beta$ la rainure initiale pour créer celle d'après, et ainsi de suite.
    ...
    \subsection{Dessin sur SolidWorks}
        ...
        \subsubsection{Dessin du doigt}
            ...
            \subsubsection{3. Esquisse des trapèzes}
            Après avoir importé l’esquisse de la spirale aplatie, incluant différentes profondeurs de trapèzes trapézoïdales espacées selon les points définis, nous traçons les lignes de construction des trapèzes en fonction des largeurs calculées à l’aide de la formule suivante :
            \begin{equation}
                \delta = ae^{b(\theta+2\pi)}-ae^{b\theta}
                \label{eq:epaisseur_trapeze}
            \end{equation}
            L’idée est de concevoir un doigt enroulable présentant une élasticité élevée, lui permettant de revenir naturellement à sa position d’équilibre. Pour cela, nous avons ajouté une couche élastique au centre de l’esquisse, d’une épaisseur équivalente à $10\%$ de la largeur d’un trapèze :
            \begin{equation}
                \delta' = \delta(1 + 10\%)
                \label{eq:epaisseur_trapeze_adjusted}
            \end{equation}

            \vspace{3cm}

            \begin{lstlisting}[style=csvstyle,caption={Largeurs des trapèzes},label={lst:epaisseur_trapeze}]
                0.0,0.7029458751722842
                0.5235987755982988,0.8458326525511634
                1.0471975511965976,1.0177638156655973
                1.5707963267948966,1.2246431742188382
                2.0943951023931953,1.4735745966562819
                2.617993877991494,1.7731059443464483
                3.141592653589793,2.133522589905259
                3.665191429188092,2.5672005985597455
                4.1887902047863905,3.0890317001697056
                4.71238898038469,3.7169346446891147
                5.235987755982988,4.472470500102403
                5.759586531581287,5.381583021070124
            \end{lstlisting}
            Nous avons ainsi obtenu un tableau des largeurs des trapèzes (deuxième colonne derrière les virgules) en fonction des valeurs de $\theta$ (première colonne devant les virgules). Nous avons constaté que l'épaisseur du doigt était relativement faible, ce qui pourrait entraîner des difficultés lors du pliage de l'objet par la pince. Par conséquent, nous avons décidé d’augmenter la base du doigt du robot à $50\,\text{mm}$, et avons déduit le facteur de correction approprié comme suit :
            \begin{equation}
                \text{Facteur de correction} = \frac{50}{\text{valeur du trapèze à } \theta_{\text{max}}}
                \label{eq:correction_factor}
            \end{equation}
            Nous obtenons donc la formule final de l'épaisseur du trapèze : 
            \begin{equation}
                \text{epaisseur} = \delta'.\ \text{Facteur de correction}
                \label{eq:epaisseur_final}
            \end{equation}

            \textbf{Esquisse des trapèzes}
            Comme mentionné dans les sections précédentes, afin de dessiner les trapèzes de manière paramétrée, nous avons défini un trapèze de référence tracé à partir des lignes de construction, en tenant compte de l’épaisseur de la couche élastique et d’une convention angulaire $\Delta \theta = 30^\circ$. Nous avons ensuite commencé l’esquisse par la base du doigt, car cela permet une mise à l’échelle plus aisée pour différentes tailles, en agrandissant progressivement chaque trapèze selon le facteur d’échelle $\beta = 1{,}203268534$, jusqu’à l’extrémité.

            \begin{figure}[H]
                \centering
                \begin{subfigure}[t]{0.4\textwidth}
                    \centering
                    \includegraphics[width=\textwidth]{Figures/Scale_trapeze_1.png}
                \end{subfigure}
                \hfill
                \begin{subfigure}[t]{0.5\textwidth}
                    \centering
                    \includegraphics[width=\textwidth]{Figures/Scale_trapeze_2.png}
                \end{subfigure}
                \caption{Mise à l'échelle de la taille des trapèzes en fonction de $\beta$}
                \label{fig:Scale_trapeze}
            \end{figure}

            Par ailleurs, il est important de noter que les deux trapèzes situés à l’extrémité du doigt présentent une géométrie particulière : leurs têtes sont plates et ne peuvent donc pas être générées selon la même méthode que les autres. Il a donc été nécessaire de les tracer séparément, afin de faciliter l’esquisse complète des lignes de construction.

            Cependant, nous avons constaté certaines limites dans cette méthode d’esquisse lorsqu’il s’agit de découper le bloc extrudé. En effet, après avoir extrudé un bloc à partir des lignes de construction (voir figure \ref{fig:construction_sketch}), nous avons obtenu un volume massif présentant des rainures bien symétriques et des surfaces latérales planes. Cette configuration rend le doigt assez volumineux, ce qui peut poser problème lors de son intégration à la base, notamment en réduisant l’espace disponible et en compliquant les opérations de préhension.

            C’est pourquoi nous avons envisagé une solution visant à affiner la forme du doigt, afin d’élargir la plage de préhension et de rendre la structure plus adaptée aux opérations de pincement.

            Pour ce faire, nous avons d’abord généré un volume trapézoïdal effilé à l’aide de la fonction Lofted Boss/Base, afin d’obtenir une forme plus pointue. Ce volume a ensuite été découpé selon l’esquisse des rainures, préalablement définie. C’est à ce stade que la méthode d’esquisse fondée sur la mise à l’échelle par $\beta$ s’est révélée inadaptée : si l’on se contente d’une extrusion simple du doigt à partir de son esquisse, cette méthode fonctionne parfaitement. En revanche, lorsqu’il s’agit de découper le bloc trapézoïdal, il est préférable de construire les rainures à partir de lignes pointillées (comme illustré dans la figure \ref{fig:construction_sketch}) et de relier les extrémités par des traits pleins pour dessiner les parois latérales, ce qui permet une découpe plus précise et conforme à la géométrie souhaitée.



            \begin{figure}[H]
                \centering
                \includegraphics[width=0.4\textwidth]{Figures/construction_sketch.png}
                \caption{Ligne de construction}
                \label{fig:construction_sketch}
            \end{figure}

            \begin{figure}[H]
                \centering
                \includegraphics[width=0.4\textwidth]{Figures/cut_sketch.png}
                \caption{Spirale Aplatie}
                \label{fig:Esquisse découpée}
            \end{figure}
...
\section{Résultats}
    ...
    \subsection{Simulation et étude de l'élasiticité via \textit{SolidWorks Simulation}}
        ...
        \subsubsection{Méthode}
            ...
            De même que \cite{bhat_numerical_2025}, nous restons dans l'hypothèse des petits déplacements pour des matériaux homogènes, isotropes et incompressibles. Nous décidons également de négliger la gravité. En effet, d'après SolidWorks, notre doigt mesure $30$ grammes. Son poids est donc d'environ $0,3$ N, or les forces des câbles seront de l'ordre de la dizaine de Newton puisque l'on souhaite attraper des objets allant jusqu'à 5 kilogrammes ($50$ Newton). Ainsi, la gravité représente moins de $1$ \% des efforts subis par le doigts.
        ...
        \textbf{Résultats}
            ...
            \[
            5{,}578\ \text{MPa} < 8{,}6\ \text{MPa} \quad \Rightarrow \quad \sigma_{\text{max}} < \sigma_{\text{limite}}
            \]
