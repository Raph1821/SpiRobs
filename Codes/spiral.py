...
for i, b in enumerate(b_values):  # lignes = b
    ...
    for j, a in enumerate(a_values):  # colonnes = a
        ...
        ax = axes[i, j]
        ax.plot(x, y, label=fr"$a_c$={a}, $b_c$={b}")
        ...
        ax.set_title(fr"$a_c$={a}, $b_c$={b}", fontsize=10)

        # Rendre les ticks plus lisibles
        ax.tick_params(axis='both', labelsize=8)

# Ajouter flèches directionnelles
fig.text(0.5, 0.98, r"$a_c$ augmente", ha='center', va='center', fontsize=14, weight='bold')
fig.text(0.095, 0.5, r"$b_c$ augmente", ha='center', va='center', fontsize=14, weight='bold', rotation=90)
